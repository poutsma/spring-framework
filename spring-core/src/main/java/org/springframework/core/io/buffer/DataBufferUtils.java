...
public abstract class DataBufferUtils {
		...
		return Flux.defer(() -> {
			...
			return Flux.from(publisher)
					.map(buffer -> {
						...
						return count >= 0 ? buffer :
								buffer.slice(0, buffer.readableByteCount() + (int) count);
					})
	...
	public static Flux<DataBuffer> skipUntilByteCount(Publisher<DataBuffer> publisher, long maxByteCount) {
		...
		return Flux.defer(() -> {
			...
			return Flux.from(publisher)
					.skipUntil(buffer -> {
						long remainder = byteCountDown.addAndGet(-buffer.readableByteCount());
						return remainder < 0;
					})
					.map(buffer -> {
	...
	public static Mono<DataBuffer> join(Publisher<DataBuffer> dataBuffers) {
		...
		return Flux.from(dataBuffers)
				...
				.doOnDiscard(PooledDataBuffer.class, DataBufferUtils::release)
				.doOnDiscard(List.class, list -> list.forEach(o -> {
					if (o instanceof PooledDataBuffer) {
						System.out.println("foo");
						DataBufferUtils.release((PooledDataBuffer) o);
					}
				}));
	}
