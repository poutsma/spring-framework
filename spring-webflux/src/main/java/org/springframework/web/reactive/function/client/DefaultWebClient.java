...
class DefaultWebClient implements WebClient {
	...
	private class DefaultHeaderSpec implements HeaderSpec {
		...
		public Mono<ClientResponse> exchange() {
			ClientRequest<Void> request = this.<Void>initRequestBuilder().build();
		...
		public <T> Mono<ClientResponse> exchange(BodyInserter<T, ? super ClientHttpRequest> inserter) {
			ClientRequest<T> request = this.<T>initRequestBuilder().body(inserter).build();
			return getExchangeFunction().exchange(request);
		}

		@Override
		public <T, S extends Publisher<T>> Mono<ClientResponse> exchange(S publisher, Class<T> elementClass) {
			ClientRequest<S> request = initRequestBuilder().headers(this.headers).body(publisher, elementClass).build();
			return getExchangeFunction().exchange(request);
		}

		private <T> ClientRequest.Builder<T> initRequestBuilder() {
			return ClientRequest.<T>method(this.httpMethod, this.uri).headers(initHeaders()).cookies(initCookies());
		}
