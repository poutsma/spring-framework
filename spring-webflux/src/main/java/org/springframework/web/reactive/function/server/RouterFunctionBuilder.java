...
class RouterFunctionBuilder implements RouterFunctions.Builder {
	...
	@Override
	public RouterFunctions.Builder before(Function<ServerRequest,
	...
	@Override
	public RouterFunctions.Builder after(BiFunction<ServerRequest, ServerResponse,
			Mono<ServerResponse>> responseProcessor) {
		return filter((request, next) -> next.handle(request)
				.flatMap(serverResponse -> responseProcessor.apply(request, serverResponse)));
	}

	@Override
	public RouterFunctions.Builder exception(Predicate<? super Throwable> predicate,
