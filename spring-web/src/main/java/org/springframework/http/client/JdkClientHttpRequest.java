...
class JdkClientHttpRequest extends AbstractStreamingClientHttpRequest {
	...
	private HttpRequest buildRequest(HttpHeaders headers, @Nullable Body body) {
		...
		if (body != null) {
			...
			if (contentLength != -1) {
				bodyPublisher = HttpRequest.BodyPublishers.fromPublisher(outputStreamPublisher, contentLength);
			}
			else {
				bodyPublisher = HttpRequest.BodyPublishers.fromPublisher(outputStreamPublisher);
			}
		}
		else {
			bodyPublisher = HttpRequest.BodyPublishers.noBody();
		}
		requestBuilder.method(this.method.name(), bodyPublisher);
