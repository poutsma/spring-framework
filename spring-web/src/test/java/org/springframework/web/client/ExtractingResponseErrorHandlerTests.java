...
public class ExtractingResponseErrorHandlerTests {
	...
	public void handleErrorStatusMatch() throws Exception {
		...
		try {
			...
			assertTrue(ex + " is not a MyRestClientException", ex instanceof MyRestClientException);
			MyRestClientException myEx = (MyRestClientException) ex;
			assertEquals("bar", myEx.getFoo());
		}
	}
	...
	public void handleNoMatch() throws Exception {
		...
		try {
			...
			assertTrue(ex + " is not a HttpClientErrorException", ex instanceof HttpClientErrorException);
			HttpClientErrorException clientEx = (HttpClientErrorException) ex;
			assertEquals(HttpStatus.NOT_FOUND, clientEx.getStatusCode());
		}
	}
