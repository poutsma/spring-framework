 ...
 */
public class ActsonJsonDecoderTests extends AbstractDataBufferAllocatingTestCase {

	private ActsonJsonDecoder decoder;

	@Before
	public void createDecoder() {
		decoder = new ActsonJsonDecoder();
	}
	...
	public void decodeSingleChunkToJsonObject() throws Exception {
		...
		Flux<String> output =
				decoder.decode(source, null, null, Collections.emptyMap())
				.map(ActsonJsonDecoderTests::toString);
	...
	public void decodeMultipleChunksToJsonObject() throws InterruptedException {
		...
		Flux<String> output =
				decoder.decode(source, null, null, Collections.emptyMap()).map(ActsonJsonDecoderTests::toString);
		ScriptedSubscriber.<String>create()
				.expectNext("{\"foo\": \"foofoo\", \"bar\": \"barbar\"}")
				.expectComplete()
				.verify(output);
